...
y_piv=-(WALL+EAR_L)-LEG_HEEL; zb=-FLOOR
C=[W(y_piv, zb), W(LEG_FOOT_Y+LEG_FOOT_L, zb-(LEG_FOOT_Y+LEG_FOOT_L-y_piv)*math.tan(th))]
ground=C[0][1]
A_P=PANEL_X*PANEL_Y/1e6; A_B=(L_IN+2*WALL)*(FLOOR+H_IN+LID_T)/1e6
...
def cfg(pose, ant, extra_mass=0.0, extra_y=None):
    ...
    if pose=="legs": it=it+[(x, y, z) for x,y,z in legs]
    if extra_mass: it=it+[(extra_mass*1000, extra_y if extra_y is not None else W_IN/2, zb+10)]
    M=sum(i[0] for i in it); cy=sum(i[0]*i[1] for i in it)/M; cz=sum(i[0]*i[2] for i in it)/M
    R=(lambda y,z: W(y,z)) if pose=="legs" else (lambda y,z: (y,z))
    Cc=[R(y_piv,zb),R(LEG_FOOT_Y+LEG_FOOT_L, zb-(LEG_FOOT_Y+LEG_FOOT_L-y_piv)*math.tan(th))] if pose=="legs" \
       else [(-(WALL+EAR_L),zb),(W_IN+WALL+EAR_L,zb)]
    g=min(c[1] for c in Cc)
    cgw=R(cy,cz)
    return dict(M=M/1000,t=t,L=L/1000,dia=dia/1000,C=[c[0] for c in Cc],cg=(cgw[0],cgw[1]-g),
                pan=(R(W_IN/2,panel_z0)[0], R(W_IN/2,panel_z0)[1]-g), sma=(R(SMA_Y,SMA_Z)[0], R(SMA_Y,SMA_Z)[1]-g))
def over(cf,v,wind):
    q=0.5*RHO_A*v*v; edge=max(cf["C"]) if wind>0 else min(cf["C"])
    n=(-math.sin(cf["t"])*wind, math.cos(cf["t"]))
    Fp=q*A_P*C_P; Fa=q*C_A*cf["L"]*cf["dia"]; Fb=q*C_B*A_B
    Mo=abs(Fp*n[0])*cf["pan"][1]/1000 + Fa*(cf["sma"][1]+cf["L"]*1000/2)/1000 + Fb*cf["cg"][1]*0.6/1000 \
...
def vtip(cf,wind):
    lo,hi=0.1,150.0
    for _ in range(70):
        mid=(lo+hi)/2; Mo,Mr,_=over(cf,mid,wind)
        if Mo<Mr: lo=mid
        else: hi=mid
    return lo
...
for ant in ANT:
    for pose in ("flat","legs"):
        ...
        for vv in (30,40):
            f=[]
            for w in (1,-1):
                Mo,Mr,_=over(cf,vv,w); f.append(max(0.0,(Mo-Mr)/((b-a)/1000)))
            pf.append(f"{max(f):3.0f} N")
        print(f"{ant:22s} {pose:6s} {cf['M']*1000:5.0f}g {cf['cg'][1]:5.0f}mm  {cf['cg'][0]-a:5.0f}/{b-cf['cg'][0]:<5.0f}mm {v:6.1f} m/s  " + " / ".join(pf))
print("\nballast needed instead of pegs (stone on/against the box, tilted pose):")
for ant in ANT:
    for vdes in (30,40):
        lo,hi=0.0,40.0
        for _ in range(50):
            mid=(lo+hi)/2; cf=cfg("legs",ant,mid,-(WALL+EAR_L)+20)
