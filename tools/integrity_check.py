...
exec(
    open(os.path.join(ROOT, "freecad", "SolarMeshtasticNodeMini_Enclosure.FCMacro"), encoding="utf-8").read(), globals()
)
...
print(
    "vent            hole %.1f dia, thread %.1f long, nut %.1f x %.1f" % (VENT_D, VENT_THREAD, VENT_NUT_D, VENT_NUT_H)
)
print(
    "vent clamp      %.1f mm of wall, %.1f mm of thread inside, %.1f mm spare"
...
print(
    "stand leg       %.1f x %.1f x %.1f mm, %.1f cm3, %d solid"
    % (leg.BoundBox.XLength, leg.BoundBox.YLength, leg.BoundBox.ZLength, leg.Volume / 1000, len(leg.Solids))
)
print(
    "stand tray      %.1f x %.1f x %.1f mm, %.1f cm3, %d solid, slot %.1f mm over a %.1f mm rail"
    % (
        tray.BoundBox.XLength,
        tray.BoundBox.YLength,
        tray.BoundBox.ZLength,
        tray.Volume / 1000,
        len(tray.Solids),
        LEG_T + 2 * BALLAST_SLOT,
        LEG_T,
    )
)
...
print(
    "plug head       %.1f dia x %.1f, %.1f mm above ground when flat"
    % (VENT_CAP_D, VENT_CAP_H, FLOOR + VENT_Z - VENT_CAP_D / 2)
)
print(
    "tunnels         Y %.1f x %.1f, X %.1f x %.1f, skin %.1f, roof %.1f/%.1f"
