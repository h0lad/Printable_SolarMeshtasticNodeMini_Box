...
def render(npz, az, el, W=1500, H=1100, SS=2, pad=0.05, fname="out.png"):
    d = np.load(npz); n = len([k for k in d.files if k.startswith("t")])
    ...
    r = np.cross(-cam, up0); r /= np.linalg.norm(r); u = np.cross(r, -cam)
    M = np.stack([r, u, cam])
    ...
    ox = w/2 - s*(mn[0]+mx[0])/2; oy = h/2 + s*(mn[1]+mx[1])/2
    zb = np.full((h,w), -1e9); col = np.ones((h,w,3)); idb = np.full((h,w), -1, np.int32); nb = np.zeros((h,w,3))
    L1 = np.array([-0.35, -0.5, 0.8]); L1/=np.linalg.norm(L1)
    L1v = M @ L1; L2v = np.array([0.6,0.3,0.75]); L2v/=np.linalg.norm(L2v)
    tri_id = 0
    for gi,(T,c) in enumerate(groups):
        P = T @ M.T                                   # view coords
        nrm = np.cross(P[:,1]-P[:,0], P[:,2]-P[:,0]); ln = np.linalg.norm(nrm,axis=1); ok = ln>1e-12
        P, nrm = P[ok], nrm[ok]/ln[ok,None]
        nrm[nrm[:,2]<0] *= -1
        dif = 0.28 + 0.55*np.clip(nrm@L1v,0,1) + 0.22*np.clip(nrm@L2v,0,1)
        Hh = (L1v+np.array([0,0,1.0])); Hh/=np.linalg.norm(Hh)
        spec = 0.22*np.clip(nrm@Hh,0,1)**40
        shade = np.clip(np.outer(dif, c) + spec[:,None], 0, 1)
        X = ox + s*P[:,:,0]; Y = oy - s*P[:,:,1]; Z = P[:,:,2]
        for k in range(len(P)):
            x0,x1,x2 = X[k]; y0,y1,y2 = Y[k]
    ...
    img.save(fname); return img
if __name__ == "__main__":
    t=time.time()
    render(sys.argv[1], float(sys.argv[2]), float(sys.argv[3]), fname=sys.argv[4]); print("t", time.time()-t)
